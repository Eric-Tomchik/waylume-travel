...
export default function AdminPage() {
  const [token,setToken]=useState(""); const [leads,setLeads]=useState<Lead[]>([]); const [authorized,setAuthorized]=useState(false); const [error,setError]=useState(""); const [loading,setLoading]=useState(false);
  async function loadLeads(e?:FormEvent){e?.preventDefault();setLoading(true);setError("");const response=await fetch("/api/admin/leads",{headers:{"x-admin-token":token},cache:"no-store"});if(!response.ok){setAuthorized(false);setError(response.status===401?"Invalid advisor passcode.":"Unable to load inquiries.");setLeads([]);}else{const data=await response.json();setAuthorized(true);setLeads(data.leads??[]);}setLoading(false);}
  async function updateLead(id:string,patch:Partial<Pick<Lead,"status"|"advisorNotes"|"followUpAt">>&{clearFollowUp?:boolean}){const response=await fetch("/api/admin/leads",{method:"PATCH",headers:{"Content-Type":"application/json","x-admin-token":token},body:JSON.stringify({id,...patch})});if(!response.ok)return setError("Unable to update inquiry.");setLeads(current=>current.map(lead=>lead._id===id?{...lead,...patch,followUpAt:patch.clearFollowUp?undefined:(patch.followUpAt??lead.followUpAt)}:lead));}
  return <main className="admin-shell"><div className="shell">
    ...
    {!authorized?<form className="admin-login" onSubmit={loadLeads}><h2>Advisor access</h2><p>Enter the admin passcode configured in the hosting environment.</p><input type="password" value={token} onChange={e=>setToken(e.target.value)} placeholder="Admin passcode" required/><button className="button" disabled={loading}>{loading?"Loading…":"Open workspace"}</button>{error&&<p className="error">{error}</p>}</form>:<div className="lead-list">
