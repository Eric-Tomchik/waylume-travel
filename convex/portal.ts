...
export const resolveAccess = query({
  ...
  handler: async (ctx, args) => {
    ...
    return {
      ...
      quotes: quotes.filter(q => q.status !== "draft").map(q => ({ _id: q._id, title: q.title, summary: q.summary, amount: q.amount, currency: q.currency, supplierName: q.supplierName, expiresAt: q.expiresAt, status: q.status })),
